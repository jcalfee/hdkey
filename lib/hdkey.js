...
HDKey.prototype.derive = function (path) {
  ...
  entries.forEach(function (c, i) {
    ...
    var numeric = /^[0-9]+'?$/.test(c)

    var childIndex
    if (numeric) {
      childIndex = parseInt(c, 10) // & (HARDENED_OFFSET - 1)
...
HDKey.prototype.deriveChild = function (index) {
  var numeric = /^[0-9]+'?$/.test(index)
  var isHardened
  var indexBuffer

  if (numeric) {
    isHardened = index >= HARDENED_OFFSET
    indexBuffer = new Buffer(4)
    indexBuffer.writeUInt32BE(index, 0)
  } else {
    isHardened = (index.length > 1) && (index[index.length - 1] === "'")
